...
- `Check` `(Check: nil)` - Specifies a check. Please see the
  [check documentation](/agent/api-v1/api-server/agent/check.html) for more information about the
...
- `EnableTagOverride` `(bool: false)` - Specifies to disable the anti-entropy
  ...
  external agents can update this service in the [catalog](/agent/api-v1/api-server/catalog.html)
