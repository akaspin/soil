...
[`Near` parameter](/agent/api/api-server/query.html#near) in the query definition
...
expecting local addresses, so a new [`X-Consul-Translate-Addresses`](/agent/api/api-server/index.html#translate_header)
