...
func TestEvaluation_Plan(t *testing.T) {
	left1 := makeAllocations("testdata/evaluation_test_left.hcl")[0]

	t.Run("0 - noop pod", func(t *testing.T) {
		right := makeAllocations("testdata/evaluation_test_left.hcl")[0]
	...
	t.Run("1 - unit-1 perm to disabled", func(t *testing.T) {
		right := makeAllocations("testdata/evaluation_test_1_right.hcl")[0]
		evaluation := scheduler.NewEvaluation(left1, right)
	...
	t.Run("2 - update unit-1 and file", func(t *testing.T) {
		right := makeAllocations("testdata/evaluation_test_2_right.hcl")[0]
		evaluation := scheduler.NewEvaluation(left1, right)
	...
	t.Run("5 - local to runtime", func(t *testing.T) {
		right := makeAllocations("testdata/evaluation_test_5_right.hcl")[0]
		evaluation := scheduler.NewEvaluation(left1, right)
