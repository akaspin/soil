...
* The [Catalog endpoints](/agent/api-v1/api-server/catalog.html) and
  [Health endpoints](/agent/api-v1/api-server/health.html) can sort the results of queries based
  on the network round trip time from a given node using a "?near=" parameter.

* [Prepared queries](/agent/api-v1/api-server/query.html) can automatically fail over services 
  to other Consul datacenters based on network round trip times.

* The [Coordinate endpoint](/agent/api-v1/api-server/coordinate.html) exposes raw network
...
[Coordinate endpoint](/agent/api-v1/api-server/coordinate.html).
